...
if __name__ == '__main__':
    ...
    np.save(os.path.join(args.out_dir, "naive_recon"), recon)
    print(f"Naive Sense recon saved in directory {args.out_dir}!")

    for bin in [4,6,10,15,25,30,60,150]:
        mask = sampling_mask(bin, recon.shape, img_axis=1)
        path = os.path.join(args.out_dir, f"sampling_mask_{bin}bins")
        np.save(path, mask)
        print(f"Sampling Mask saved in {path}!")
